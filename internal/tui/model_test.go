...
func newMockBulk() *mockBulk {
	return &mockBulk{entries: make(map[string]map[string]string)}
}

func (m *mockBulk) BulkSet(entries map[string]string, vault string) (int, error) {
	if m.entries[vault] == nil {
		m.entries[vault] = make(map[string]string)
	}
	cloned := make(map[string]string, len(entries))
	for key, value := range entries {
		m.entries[vault][key] = value
		cloned[key] = value
	}
	m.bulkCalls = append(m.bulkCalls, bulkCall{vault: vault, entries: cloned})
	return len(entries), nil
}

func (m *mockBulk) GetAll(vault string) (map[string]string, error) {
	cloned := make(map[string]string)
	for key, value := range m.entries[vault] {
		cloned[key] = value
	}
	return cloned, nil
}
...
func TestHelpOverlayDismissesOnAnyKey(t *testing.T) {
	...
	updated, _ = model.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'?'}})
	model = updated.(Model)
	if model.mode != modeOverlay {
		t.Fatalf("mode after ? = %v, want modeOverlay", model.mode)
	}
	if model.overlay.kind != overlayHelp {
		t.Fatalf("overlay kind = %v, want overlayHelp", model.overlay.kind)
	}

	updated, _ = model.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'a'}})
	model = updated.(Model)
	if model.mode != modeBrowse {
		t.Fatalf("mode after dismiss = %v, want modeBrowse", model.mode)
	}
}

func TestVaultManagementSwitchesVault(t *testing.T) {
	vaults := &mockVaults{list: []string{"default", "prod"}, active: "default"}
	m := NewModel(Deps{Store: newMockStore(), Bulk: newMockBulk(), Vaults: vaults})
	updated, _ := m.Update(loadedMsg{
		vaults:      []string{"default", "prod"},
		activeVault: "default",
		items:       []entry{{Vault: "default", Key: "A"}, {Vault: "prod", Key: "B"}},
	...
	model = updated.(Model)
	if model.overlay.kind != overlayVaultManage {
		t.Fatalf("overlay kind = %v, want overlayVaultManage", model.overlay.kind)
	}
	...
	model = updated.(Model)
	if cmd == nil {
		t.Fatal("expected reload command after switching vault")
	}
	if len(vaults.switchCalls) != 1 || vaults.switchCalls[0] != "prod" {
		t.Fatalf("switch calls = %#v, want [prod]", vaults.switchCalls)
	}
	updated, _ = model.Update(cmd())
	model = updated.(Model)
	if model.activeVault != "prod" {
		t.Fatalf("active vault = %q, want prod", model.activeVault)
	}
	if model.currentFilter != "prod" {
		t.Fatalf("current filter = %q, want prod", model.currentFilter)
	}
}

func TestCreateVaultFromBrowseCreatesAndSwitches(t *testing.T) {
	...
	model = updated.(Model)
	model.inputField.SetValue("prod")
	...
	model = updated.(Model)
	if cmd == nil {
		t.Fatal("expected reload command after create")
	}
	updated, _ = model.Update(cmd())
	model = updated.(Model)

	if len(vaults.createCalls) != 1 || vaults.createCalls[0] != "prod" {
		t.Fatalf("create calls = %#v, want [prod]", vaults.createCalls)
	}
	if model.activeVault != "prod" || model.currentFilter != "prod" {
		t.Fatalf("vault state = active:%q filter:%q, want prod/prod", model.activeVault, model.currentFilter)
	}
	if !strings.Contains(model.flashMessage, "Created vault prod") {
		t.Fatalf("flashMessage = %q, want create confirmation", model.flashMessage)
	}
}

func TestImportEnvFileIntoActiveVault(t *testing.T) {
	store := newMockStore()
	bulk := newMockBulk()
	vaults := &mockVaults{list: []string{"default"}, active: "default"}
	m := NewModel(Deps{Store: store, Bulk: bulk, Vaults: vaults, Clipboard: &mockClipboard{}})
	updated, _ := m.Update(loadedMsg{vaults: []string{"default"}, activeVault: "default"})
	model := updated.(Model)
	...
	if _, err := file.WriteString("API_KEY=secret\nDB_PASS=hidden\n"); err != nil {
	...
	model = updated.(Model)
	model.inputField.SetValue(file.Name())
	...
	model = updated.(Model)
	if cmd == nil {
		t.Fatal("expected reload command after import")
	}
	store.keys["default"] = []string{"API_KEY", "DB_PASS"}
	store.metadata["default"] = map[string]string{"API_KEY": protectionProtected, "DB_PASS": protectionProtected}
	updated, _ = model.Update(cmd())
	model = updated.(Model)

	if len(bulk.bulkCalls) != 1 {
		t.Fatalf("bulk calls = %d, want 1", len(bulk.bulkCalls))
	}
	if bulk.bulkCalls[0].vault != "default" {
		t.Fatalf("bulk call vault = %q, want default", bulk.bulkCalls[0].vault)
	}
	if !strings.Contains(model.flashMessage, "Imported 2 keys into vault default") {
		t.Fatalf("flashMessage = %q, want import confirmation", model.flashMessage)
	}
}

func TestExportPreviewMasksValuesAndFileExportWritesEnv(t *testing.T) {
	bulk := newMockBulk()
	bulk.entries["prod"] = map[string]string{"API_KEY": "secret", "DB_PASS": "hidden"}
	store := newMockStore()
	store.keys["prod"] = []string{"API_KEY", "DB_PASS"}
	store.metadata["prod"] = map[string]string{"API_KEY": protectionProtected, "DB_PASS": protectionProtected}
	vaults := &mockVaults{list: []string{"default", "prod"}, active: "prod"}
	m := NewModel(Deps{Store: store, Bulk: bulk, Vaults: vaults, Clipboard: &mockClipboard{}})
	updated, _ := m.Update(loadedMsg{vaults: []string{"default", "prod"}, activeVault: "prod", items: []entry{{Vault: "prod", Key: "API_KEY"}, {Vault: "prod", Key: "DB_PASS"}}})
	model := updated.(Model)
	model.currentFilter = "prod"
	model.applyFilters()
	...
	model = updated.(Model)
	if cmd != nil {
		t.Fatal("masked preview should not require export command")
	}
	preview := model.previewView()
	if !strings.Contains(preview, "API_KEY=****") || strings.Contains(preview, "secret") {
		t.Fatalf("previewView = %q, want masked export preview", preview)
	}

	path := t.TempDir() + "/backup.env"
	updated, _ = model.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'X'}})
	model = updated.(Model)
	model.inputField.SetValue(path)
	updated, cmd = model.Update(tea.KeyMsg{Type: tea.KeyEnter})
	model = updated.(Model)
	if cmd == nil {
		t.Fatal("expected file export command")
	}
	updated, _ = model.Update(cmd())
	model = updated.(Model)
	...
	if !strings.Contains(string(data), "API_KEY=secret") || !strings.Contains(string(data), "DB_PASS=hidden") {
		t.Fatalf("exported file = %q, want env lines", string(data))
	}
	if !strings.Contains(model.flashMessage, "Exported 2 keys to "+path) {
		t.Fatalf("flashMessage = %q, want export confirmation", model.flashMessage)
	}
}

func TestLoadVaultCopiesShellCommand(t *testing.T) {
	clipboard := &mockClipboard{}
	m := NewModel(Deps{Store: newMockStore(), Bulk: newMockBulk(), Vaults: &mockVaults{list: []string{"default", "prod"}, active: "default"}, Clipboard: clipboard})
	updated, _ := m.Update(loadedMsg{vaults: []string{"default", "prod"}, activeVault: "default"})
	model := updated.(Model)
	...
	if len(clipboard.values) != 1 || clipboard.values[0] != `eval "$(kc env --vault prod)"` {
		t.Fatalf("clipboard values = %#v, want eval command for prod", clipboard.values)
	}
	preview := model.previewView()
	if !strings.Contains(preview, `Run: eval "$(kc env --vault prod)"`) {
		t.Fatalf("previewView = %q, want load command instructions", preview)
	}
	if !strings.Contains(model.flashMessage, "Load command copied to clipboard") {
		t.Fatalf("flashMessage = %q, want load confirmation", model.flashMessage)
	}
}

func TestDeleteVaultRequiresActiveFilterAndSwitchesToDefault(t *testing.T) {
	vaults := &mockVaults{list: []string{"default", "prod"}, active: "prod"}
	m := NewModel(Deps{Store: newMockStore(), Bulk: newMockBulk(), Vaults: vaults})
	updated, _ := m.Update(loadedMsg{vaults: []string{"default", "prod"}, activeVault: "prod", items: []entry{{Vault: "prod", Key: "TOKEN"}}})
	model := updated.(Model)

	updated, _ = model.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'D'}})
	model = updated.(Model)
	if model.mode != modeBrowse {
		t.Fatalf("mode with all-vaults delete = %v, want modeBrowse", model.mode)
	}

	model.currentFilter = "prod"
	updated, _ = model.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'D'}})
	model = updated.(Model)
	if model.overlay.kind != overlayConfirmVaultDelete {
		t.Fatalf("overlay kind = %v, want overlayConfirmVaultDelete", model.overlay.kind)
	}
	updated, cmd := model.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'y'}})
	model = updated.(Model)
	if cmd == nil {
		t.Fatal("expected delete vault command")
	}
	updated, cmd = model.Update(cmd())
	model = updated.(Model)
	if cmd == nil {
		t.Fatal("expected reload command after vault delete")
	}
	updated, _ = model.Update(cmd())
	model = updated.(Model)

	if len(vaults.deleteCalls) != 1 || !vaults.deleteCalls[0].force || vaults.deleteCalls[0].name != "prod" {
		t.Fatalf("delete calls = %#v, want force delete of prod", vaults.deleteCalls)
	}
	if model.activeVault != "default" || model.currentFilter != "default" {
		t.Fatalf("vault state after delete = active:%q filter:%q, want default/default", model.activeVault, model.currentFilter)
	}
}
